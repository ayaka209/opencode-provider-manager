...
- Clarified README guidance for extending an existing provider versus adding a new provider.
- Added README examples for importing provider/model updates from GitHub branches and unmerged PRs.
- Aligned CLI help and documentation so read-oriented commands default to `merged`, while TUI editing defaults to the `project` layer.
